...
const getProtocolCardDetails = protocolId => {
  ...
  const name = protocol.name || "No Protocol";
  const summary = protocol.summary || "No Protocol prepared.";
  const bonuses = Array.isArray(protocol.bonuses) && protocol.bonuses.length
    ? "\n\nBonuses:\n" + protocol.bonuses.map(bonus => "• " + bonus).join("\n")
    : "";
  return {
    display: name,
...
on("change:repeating_protocols:protocol_name", eventInfo => {
  ...
  const match = source.match(/^repeating_protocols_([^_]+)_protocol_name$/);
  if (!match) return;
  const rowId = match[1];
  const prefix = `repeating_protocols_${rowId}`;
  getAttrs([`${prefix}_protocol_name`], values => {
    const selectedId = values[`${prefix}_protocol_name`] || "";
