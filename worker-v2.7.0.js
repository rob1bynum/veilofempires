  ...
  getAttrs(fields, values => {
    ...
    [1,2,3].forEach(slot => {
      ...
      updates[`protocol_slot_${slot}_display`] = key && selectedRank > 0
        ? `${protocol.name} ${selectedRank >= 2 ? "II" : "I"}`
        : "No Protocol";
...
const recalculateCharacter = () => {
  ...
  getAttrs([...new Set(fields)], values => {
    ...
    const filledXp = Math.max(0,Math.min(10,Math.floor((currentXp/nextXp)*10)));
...
const getProtocolCardDetails = (protocolId, selectedRank, agentRank) => {
  ...
  return {
    rank:safeRank,
    display: protocolId && safeRank > 0 ? `${protocol.name} ${safeRank >= 2 ? "II" : "I"}` : "No Protocol",
...
on("remove:repeating_protocols", refreshProtocolBudgetWithRepeating);
