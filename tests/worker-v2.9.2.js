...
const protocolForSelectedRank = (key, selectedRank) => {
  ...
  return selectedRank >= 2 && PROTOCOLS_RANK_2[key]
    ? PROTOCOLS_RANK_2[key]
...
const refreshWeaponDamageDisplay = () => {
  getAttrs(["active_weapon","armor_type","incoming_damage"],values=>{
...
const rollActiveWeaponDamage = () => {
  getAttrs(["character_name","active_weapon"],values=>{
    const weapon = getActiveWeapon(values.active_weapon || "unarmed");
    ...
    startRoll(
      `&{template:voe} {{name=${escapeTemplate(weapon.name)}}} {{agent=${escapeTemplate(values.character_name||"Unnamed Agent")}}} {{kind=Weapon Damage}} {{showdamage=1}} {{damage=[[${expression}]]}} {{damageformula=${escapeTemplate(weapon.damage)}}} {{damagetype=${escapeTemplate(weapon.type)}}} {{traits=${escapeTemplate(weapon.traits)}}}`,
      results=>{
        const damage = Math.max(0,Math.trunc(results.results.damage.result));
        finishRoll(results.rollId,{damage});
      }
    );
  });
};
...
on("change:active_weapon change:armor_type change:incoming_damage", refreshWeaponDamageDisplay);
...
const runCombatAttack = mode => {
  ...
  getAttrs([field,"actions_current","attacks_made","armor_penalty","target_armor_type","target_evading","attack_other_modifier","protocol_slot_1","protocol_slot_2","protocol_slot_3",...CONDITION_KEYS.map(key=>`condition_${key}`)],v=>{
    ...
    const base=numberValue(v[field],0), multi=(mode==="close" && hasBattleFocus && attacks===1 ? 0 : attacks*10), evasion=v.target_evading==="1"?10:0, other=numberValue(v.attack_other_modifier,0);
    ...
    const finalTarget=clampPercent(base+conditionModifier+armorDifference-multi-evasion+other);
    setAttrs({armor_difference_modifier:armorDifference},{silent:true});
    runPercentileTest({name:label,target:finalTarget,kind:"Attack",description:`Base ${Math.trunc(base)}% | Conditions ${conditionModifier>=0?"+":""}${conditionModifier}% | Armor Difference ${armorDifference>=0?"+":""}${armorDifference}% | Attack ${attacks+1} penalty -${multi}% | Evasion -${evasion}% | Other ${other>=0?"+":""}${other}% | Final ${finalTarget}%`});
...
on("clicked:evasion",()=>getAttrs(["reaction_available","athletics_total",...CONDITION_KEYS.map(key=>`condition_${key}`)],v=>{
  if(v.reaction_available!=="1") return postNotice("Evasion","Your Reaction has already been spent this round.","Reaction");
  const normalTarget = clampPercent(numberValue(v.athletics_total,0));
  const active = getActiveConditions(v);
  const conditionModifier =
    conditionPenaltyForAttribute("strength",active) +
    conditionPenaltyForSkill("athletics","strength",active);
  const adjustedTarget = clampPercent(normalTarget + conditionModifier);
  setAttrs({reaction_available:0},{silent:true});
  runPercentileTest({
    name:"Evasion",
    target:adjustedTarget,
    kind:"Reaction • Athletics",
    description:`Normal ${normalTarget}%${conditionModifier ? ` | Conditions ${conditionModifier}% | Adjusted ${adjustedTarget}%` : ""}. On a success, the incoming attacker reduces their attack target by 10%. Declare before the attack roll.`
  });
}));
...
on("clicked:reset_round",()=>{setAttrs({actions_current:3,attacks_made:0,attacks_remaining:3,reaction_available:1,target_evading:0,armor_difference_modifier:0},{silent:true});postNotice("New Round","Actions reset to 3, attack penalties cleared, and Reaction restored.");});
