...
const refreshBuilderFinalReview = (callback=null) => {
  recalculateCharacter(()=>{
    refreshBuilderSelections(()=>{
      refreshCreationBudgets(()=>{
        refreshAllProtocolState(()=>{
          validateCharacterBuilder(()=>{
            setAttrs({
              builder_wizard_message:"Review every choice below, then select Finish Character."
            },{silent:true},()=>{
              if(callback) callback();
            });
          });
...
attrs.protocol_slot_1="ghost_step"; attrs.protocol_rank_1="1";
attrs.protocol_slot_2="arcane_sight"; attrs.protocol_rank_2="1";
...
attrs.strength_base="5";attrs.dexterity_base="5";attrs.endurance_base="5";
attrs.intelligence_base="5";attrs.wisdom_base="5";attrs.charisma_base="5";
trigger("clicked:wizard_next",{});
if(String(attrs.builder_step)!=="7") throw new Error("Wizard did not advance to Step 7");
if(attrs.builder_review_heritage!=="Rajah") throw new Error("Review Heritage missing");
if(attrs.builder_review_affiliation!=="House Veylan") throw new Error("Review Affiliation missing");
if(attrs.builder_review_specialty!=="Investigator") throw new Error("Review Specialty missing");
console.log(JSON.stringify({step:attrs.builder_step,heritage:attrs.builder_review_heritage,affiliation:attrs.builder_review_affiliation,specialty:attrs.builder_review_specialty,validation:attrs.builder_validation_state,message:attrs.builder_wizard_message}));
