...
const runCombatAttack = mode => {
  ...
  getAttrs([field,"actions_current","attacks_made","armor_penalty","target_armor_type","attack_other_modifier","protocol_slot_1","protocol_slot_2","protocol_slot_3",...CONDITION_KEYS.map(key=>`condition_${key}`)],v=>{
    ...
    const finalTarget=clampPercent(base+conditionModifier+armorDifference-multi+other);
    setAttrs({armor_difference_modifier:armorDifference},{silent:true});
    runPercentileTest({name:label,target:finalTarget,kind:"Attack",description:`Base ${Math.trunc(base)}% | Conditions ${conditionModifier>=0?"+":""}${conditionModifier}% | Armor Difference ${armorDifference>=0?"+":""}${armorDifference}% | Attack ${attacks+1} penalty -${multi}% | Other ${other>=0?"+":""}${other}% | Final ${finalTarget}%`});
...
on("clicked:reset_round",()=>{setAttrs({actions_current:3,attacks_made:0,attacks_remaining:3,reaction_available:1,armor_difference_modifier:0},{silent:true});postNotice("New Round","Actions reset to 3, attack penalties cleared, and Reaction restored.");});
