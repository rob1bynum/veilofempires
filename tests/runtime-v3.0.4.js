...
function getAttrs(fields,cb){const out={};fields.forEach(f=>out[f]=attrs[f]??"");cb(out);}
...
attrs.heritage_mirror="rajah";
attrs.heritage="rajah";
attrs.specialty="";
attrs.rank="1";
attrs.health="10";
attrs.health_max="10";
attrs.actions_current="3";
attrs.reaction_available="1";
attrs.condition_incapacitated="0";
attrs.condition_stunned="0";
...
trigger("change:heritage_mirror",{newValue:"rajah"});
if(Number(attrs.speed)!==30) throw new Error(`Movement expected 30, got ${attrs.speed}`);

attrs.condition_incapacitated="1";
trigger("change:condition_incapacitated",{newValue:"1"});
if(Number(attrs.actions_current)!==0) throw new Error("Incapacitated did not set Actions to 0");
if(String(attrs.reaction_available)!=="0") throw new Error("Incapacitated did not remove Reaction");

attrs.condition_incapacitated="0";
trigger("change:condition_incapacitated",{newValue:"0"});
if(Number(attrs.actions_current)!==3) throw new Error("Actions were not restored after Incapacitated");
if(String(attrs.reaction_available)!=="1") throw new Error("Reaction was not restored after Incapacitated");
console.log(JSON.stringify({movement:attrs.speed,actions:attrs.actions_current,reaction:attrs.reaction_available}));
