...
const refreshAffiliation = (grantProtocol=false,callback=null) => {
  getAttrs([
    ...
    const protocol = PROTOCOLS[affiliation.protocol] || PROTOCOLS[""];
    const updates = {
      character_summary_name:characterSummary.agentName,
      character_summary_identity:characterSummary.identity,
      character_summary_affiliation:characterSummary.affiliationLine,
      character_summary_description:characterSummary.profile,
...
on("change:heritage change:heritage_mirror change:affiliation_city change:affiliation change:specialty",refreshBuilderReviewIdentity);
on("sheet:opened",()=>refreshAffiliation(false));
on("sheet:opened",()=>setAttrs({npc_sheet_version:1},{silent:true}));
on("sheet:opened",()=>{
...
on("change:affiliation_city",()=>refreshAffiliation(true));
on("change:affiliation",()=>refreshAffiliation(true));
