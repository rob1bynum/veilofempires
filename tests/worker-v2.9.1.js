...
const runCombatAttack = mode => {
  ...
  getAttrs([field,"actions_current","attacks_made","target_armor_penalty","target_evading","attack_other_modifier","protocol_slot_1","protocol_slot_2","protocol_slot_3",...CONDITION_KEYS.map(key=>`condition_${key}`)],v=>{
    ...
    const finalTarget=clampPercent(base+conditionModifier-multi-armor-evasion+other);
    runPercentileTest({name:label,target:finalTarget,kind:"Attack",description:`Base ${Math.trunc(base)}% | Conditions ${conditionModifier>=0?"+":""}${conditionModifier}% | Attack ${attacks+1} penalty -${multi}% | Armor -${armor}% | Evasion -${evasion}% | Other ${other>=0?"+":""}${other}% | Final ${finalTarget}%`});
