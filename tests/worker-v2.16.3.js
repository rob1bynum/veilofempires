    ...
    getSectionIDs("repeating_equipment", equipmentIds => {
      ...
      getAttrs([
        ...
        setAttrs({
          ...
          export_status:`Export ready: ${weapons.length} weapon${weapons.length===1?"":"s"}, ${equipment.length} equipment item${equipment.length===1?"":"s"}, and ${activeConditions.length} active condition${activeConditions.length===1?"":"s"}. Press Ctrl + P or Command + P to save the PDF.`
        },{silent:true},()=>{
          if (callback) callback();
        });
      });
