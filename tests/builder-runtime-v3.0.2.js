...
function on(events, cb) {
  String(events).split(/\s+/).filter(Boolean).forEach(e => {
    (handlers[e] ||= []).push(cb);
  });
}
function getAttrs(fields, cb) {
  const out = {};
  fields.forEach(f => out[f] = attrs[f] ?? "");
  cb(out);
}
function setAttrs(updates, options, cb) {
  if (typeof options === "function") { cb = options; options = {}; }
  Object.assign(attrs, updates);
  if (cb) cb();
}
function getSectionIDs(section, cb) { cb([]); }
function generateRowID() { return "row1"; }
function startRoll(expr, cb) { cb({rollId:"1",results:{result:42}}); }
function finishRoll() {}
function removeRepeatingRow() {}
function getTranslationByKey(k){ return k; }
function setDefaultToken() {}
function trigger(name, info={}) {
  (handlers[name] || []).forEach(fn => fn(info));
}
...
trigger("change:heritage_mirror",{newValue:"rajah",sourceAttribute:"heritage_mirror"});
if(attrs.heritage_display !== "Rajah") throw new Error("Heritage preview failed");

attrs.affiliation_city = "imperium";
trigger("change:affiliation_city",{newValue:"imperium"});
attrs.builder_affiliation_imperium = "house_veylan";
trigger("change:builder_affiliation_imperium",{newValue:"house_veylan"});
if(attrs.affiliation_display !== "House Veylan") throw new Error("Affiliation preview failed");

attrs.specialty = "investigator";
trigger("change:specialty",{newValue:"investigator"});
if(attrs.specialty_display !== "Investigator") throw new Error("Specialty preview failed");

if(!attrs.builder_signature_protocol_name || attrs.builder_signature_protocol_name === "No Signature Protocol")
  throw new Error("Signature Protocol preview failed");

console.log(JSON.stringify({
  heritage:attrs.heritage_display,
  affiliation:attrs.affiliation_display,
  specialty:attrs.specialty_display,
