    ...
    getAttrs(fields, values => {
      ...
      [1,2,3].forEach(slot => {
        ...
        const selectedRank = key
          ? clampProtocolRank(values[`protocol_rank_${slot}`],arcaneRank)
          : 0;

        if (key && selectedRank > 0) {
          const protocol = protocolForSelectedRank(key,selectedRank);
          if (protocol && protocol.name && protocol.name !== "None") names.push(protocol.name);
        }
      });
      ...
      rowIds.forEach(rowId => {
        ...
        const selectedRank = key
          ? clampProtocolRank(values[`repeating_protocols_${rowId}_protocol_rank`],arcaneRank)
          : 0;
      ...
      setAttrs({
        agent_protocol_slot_1_text:visible[0] || "",
        agent_protocol_slot_2_text:visible[1] || "",
        agent_protocol_slot_3_text:visible[2] || "",
        agent_protocol_extra_text:visible.length > 3 ? visible.slice(3).join(" • ") : ""
      }, {silent:true});
    });
  });
};
...
on("change:rank change:protocol_slot_1 change:protocol_rank_1 change:protocol_slot_2 change:protocol_rank_2 change:protocol_slot_3 change:protocol_rank_3", refreshAllProtocolState);
on("sheet:opened", refreshAllProtocolState);
...
const CONDITION_LABELS = {
  ...
  shocked:"Shocked: -10% Intelligence- and Wisdom-based tests and no Reaction.",
...
const REACTION_LIBRARY = Object.freeze({
  glimpse_future:Object.freeze({
    ...
    modes:Object.freeze({forewarned:"Forewarned",exploit:"Exploit the Opening"}),
    description:"Choose -5% per Protocol Rank against one incoming attack, or +5% per Rank on your next attack."
  }),
  coiled_reflexes:Object.freeze({
    name:"Coiled Reflexes",sourceType:"heritage",sourceKey:"rajah",
    description:"When targeted by a Close Combat attack, impose -5% on that attack."
  }),
  predators_turn:Object.freeze({
    name:"Predator's Turn",sourceType:"heritage",sourceKey:"balam",
    description:"After an enemy misses you in Close Combat, move up to 5 feet."
  }),
  warning_cry:Object.freeze({
    name:"Warning Cry",sourceType:"heritage",sourceKey:"hrafn",
    description:"When an ally within 30 feet is targeted, impose -5% on that attack."
  }),
  stand_firm:Object.freeze({
    name:"Stand Firm",sourceType:"heritage",sourceKey:"ursa",
    description:"Ignore one forced-movement or knockdown effect."
  }),
  slipstream:Object.freeze({
    name:"Slipstream",sourceType:"heritage",sourceKey:"zephyr",
    description:"After you are targeted by an attack, move up to 5 feet after it resolves."
  }),
  read_the_tell:Object.freeze({
    name:"Read the Tell",sourceType:"specialty",sourceKey:"investigator",
    description:"After observing an attacker, impose -5% on one attack they make."
  }),
  vanish:Object.freeze({
    name:"Vanish",sourceType:"specialty",sourceKey:"infiltrator",
    description:"After an attack against you resolves, move up to 5 feet into available cover or concealment."
  }),
  brace:Object.freeze({
    name:"Brace",sourceType:"specialty",sourceKey:"vanguard",
    description:"Gain +1 Damage Reduction against the next hit before your next turn.",
    damageReduction:1
  }),
  rapid_analysis:Object.freeze({
    name:"Rapid Analysis",sourceType:"specialty",sourceKey:"scholar",
  ...
  scatter:Object.freeze({
    name:"Scatter",sourceType:"specialty",sourceKey:"pathfinder",
    description:"When an area hazard or attack is declared, move up to 5 feet before it resolves."
  }),
  intercept:Object.freeze({
    name:"Intercept",sourceType:"specialty",sourceKey:"warden",
    description:"Become the target of an attack aimed at an adjacent ally."
  }),
  phase_step:Object.freeze({
    name:"Phase Step",sourceType:"protocol",sourceKey:"ghost_step",
    description:"After you are targeted, move up to 10 feet after the attack resolves."
  }),
  read_the_weave:Object.freeze({
    name:"Read the Weave",sourceType:"protocol",sourceKey:"arcane_sight",
    description:"Gain +5% per Protocol Rank to identify or resist one Protocol as it is used."
  }),
  harden:Object.freeze({
    name:"Harden",sourceType:"protocol",sourceKey:"iron_heart",
    description:"Gain +2 Damage Reduction per Protocol Rank against the next hit.",
    damageReductionPerRank:2
  }),
  anchor:Object.freeze({
    name:"Anchor",sourceType:"protocol",sourceKey:"titan_frame",
    description:"Ignore one forced-movement, knockdown, or displacement effect."
  }),
  center_mind:Object.freeze({
    name:"Center Mind",sourceType:"protocol",sourceKey:"calm_mind",
    description:"Immediately clear Frightened or Charmed."
  }),
  measured_guard:Object.freeze({
    name:"Measured Guard",sourceType:"protocol",sourceKey:"battle_focus",
    description:"After you are hit in Close Combat, gain +5% per Protocol Rank on your next Close Combat attack."
  })
});
...
const reactionEffectText = (key, rank) => {
  const reaction = REACTION_LIBRARY[key];
  if (!reaction || rank <= 0) return "Reaction unavailable for this Agent.";
  if (key === "glimpse_future") {
    return `Forewarned -${rank*5}% or Exploit the Opening +${rank*5}%.`;
  }
  if (key === "read_the_weave") return `+${rank*5}% to identify or resist one Protocol.`;
  if (key === "harden") return `+${rank*2} Damage Reduction against the next hit.`;
  if (key === "measured_guard") return `+${rank*5}% to your next Close Combat attack after being hit.`;
...
const refreshReactionState = () => {
  getSectionIDs("repeating_protocols",rowIds=>{
    ...
    getAttrs(fields,values=>{
      ...
      let modifier = 0;
      if (selected === "glimpse_future") modifier = sourceRank*5;
      else if (selected === "read_the_weave") modifier = sourceRank*5;
      else if (selected === "harden") modifier = sourceRank*2;
      else if (selected === "measured_guard") modifier = sourceRank*5;
      ...
      setAttrs({
        ...
        selected_reaction_description:selected === "none"
          ...
          : reactionEffectText(selected,sourceRank),
...
const useSelectedReaction = () => {
  getSectionIDs("repeating_protocols",rowIds=>{
    ...
    getAttrs(fields,values=>{
      ...
      const mode = values.selected_reaction_mode || "forewarned";
      let effect = reactionEffectText(selected,sourceRank);
      const updates = {
      ...
      if (selected === "glimpse_future") {
        ...
        if (mode === "exploit") {
          updates.reaction_pending_attack_bonus = modifier;
          effect = `Exploit the Opening: your next attack gains +${modifier}%.`;
        } else {
          updates.reaction_pending_defense_penalty = -modifier;
          effect = `Forewarned: one attack against you suffers -${modifier}%.`;
        }
        ...
        updates.reaction_pending_attack_bonus = sourceRank*5;
      }

      setAttrs(updates);
      startRoll(
        `&{template:voe} {{name=Reaction}} {{agent=${escapeTemplate(values.character_name||"Unnamed Agent")}}} {{kind=${escapeTemplate(reaction.name)}}} {{description=${escapeTemplate(effect)} • Reaction Spent}}`,
...
ATTRIBUTE_KEYS.forEach(k=>bindAttributeTest(k,k.charAt(0).toUpperCase()+k.slice(1)));
...
const runInitiative = () => {
  const fields = ["character_name",...CONDITION_KEYS.map(key=>`condition_${key}`)];
  getAttrs(fields,values=>{
    const active = getActiveConditions(values);
    const penalty = conditionInitiativePenalty(active);
    const rollString = `&{template:voe} {{name=Initiative}} {{agent=${escapeTemplate(values.character_name||"Unnamed Agent")}}} {{kind=Initiative}} {{showinitiative=1}} {{rolled=[[1d100]]}} {{result=[[0]]}}${penalty ? ` {{description=${penalty} Condition modifier applied.}}` : ""}`;
    startRoll(rollString,results=>{
      const raw = Math.trunc(results.results.rolled.result);
      const finalResult = Math.max(1,Math.min(100,raw + penalty));
      finishRoll(results.rollId,{result:finalResult});
    });
  });
};
...
const runCombatAttack = mode => {
  ...
  getAttrs([field,"actions_current","attacks_made","armor_penalty","target_armor_type","attack_other_modifier","reaction_pending_attack_bonus","protocol_slot_1","protocol_slot_2","protocol_slot_3",...CONDITION_KEYS.map(key=>`condition_${key}`)],v=>{
    ...
    const reactionBonus = Math.max(0,numberValue(v.reaction_pending_attack_bonus,0));
    const finalTarget=clampPercent(base+conditionModifier+armorDifference+reactionBonus-multi+other);
    setAttrs({armor_difference_modifier:armorDifference,reaction_pending_attack_bonus:0},{silent:true});
...
const updateRepeatingProtocolRow = (rowId, selectedId, selectedRank, agentRank) => {
  if (!rowId) return;
  const prefix = `repeating_protocols_${rowId}`;
  const selected = getProtocolCardDetails(selectedId || "", selectedRank, agentRank);
  setAttrs({
    [`${prefix}_protocol_rank`]:selected.rank,
    [`${prefix}_protocol_display`]:selected.display,
    [`${prefix}_protocol_summary`]:selected.summary
  }, {silent:true}, refreshAgentProtocolLoadoutSummary);
};
...
on("change:repeating_protocols:protocol_name change:repeating_protocols:protocol_rank", eventInfo => {
  ...
  getAttrs(["rank",`${prefix}_protocol_name`,`${prefix}_protocol_rank`], values => {
    updateRepeatingProtocolRow(
      rowId,
      values[`${prefix}_protocol_name`] || "",
      values[`${prefix}_protocol_rank`] || 0,
      values.rank || 1
    );
  });
    ...
    getAttrs(fields, values => {
      rowIds.forEach(rowId => {
        updateRepeatingProtocolRow(
          ...
          values[`repeating_protocols_${rowId}_protocol_rank`] || 0,
          values.rank || 1
        );
      });
    });
...
const refreshProtocolBudgetWithRepeating = () => {
  getSectionIDs("repeating_protocols", rowIds => {
    ...
    getAttrs(fields, values => {
      ...
      rowIds.forEach(rowId => {
        const key = values[`repeating_protocols_${rowId}_protocol_name`] || "";
        const selectedRank = key ? clampProtocolRank(values[`repeating_protocols_${rowId}_protocol_rank`],budget.arcaneRank) : 0;
