...
const SKILL_ATTRIBUTE_MAP = {
  ...
  medicine: ["medicine", "intelligence"], survival: ["survival", "endurance"], perception: ["skill_perception", "wisdom"],
...
const refreshCreationBudgets = () => {
  ...
  getAttrs([...attributeFields,...skillFields], values => {
    ...
    setAttrs({
      ...
      attribute_points_remaining:`${Math.max(0,ATTRIBUTE_POINT_BUDGET-attributeUsed)} remaining`,
      skill_points_used:skillUsed,
      skill_points_remaining:`${Math.max(0,SKILL_POINT_BUDGET-skillUsed)} remaining`
    },{silent:true});
  });
};
...
const enforceSkillBudget = eventInfo => {
  ...
  getAttrs(skillFields, values => {
    ...
    const corrected = Math.min(100,BASE_SKILL_VALUE+allowedPoints,requested);
...
const refreshBuilderDisplay = () => {
  const fields = [
    ...
    "attribute_points_remaining","skill_points_remaining"
  ];
  getAttrs([...new Set(fields)], values => {
    const updates = {};
    ATTRIBUTE_KEYS.forEach(key=>{
      const heritage = numberValue(values[`${key}_heritage_applied`],0);
      const playerValue = Math.max(BASE_ATTRIBUTE_VALUE,numberValue(values[`${key}_base`],BASE_ATTRIBUTE_VALUE)-heritage);
    ...
    Object.entries(SKILL_ATTRIBUTE_MAP).forEach(([skill,[baseField]])=>{
      updates[`builder_skill_${skill}_display`] = Math.max(BASE_SKILL_VALUE,numberValue(values[baseField],BASE_SKILL_VALUE));
      updates[`builder_skill_${skill}_final_text`] = `Final ${numberValue(values[`${skill}_total`],0)}%`;
    });
    setAttrs(updates,{silent:true},validateCharacterBuilder);
  });
};
...
const adjustBuilderAttribute = (key,delta) => {
  ...
  getAttrs(fields, values => {
    const heritage = numberValue(values[`${key}_heritage_applied`],0);
    const currentPlayer = Math.max(BASE_ATTRIBUTE_VALUE,numberValue(values[`${key}_base`],BASE_ATTRIBUTE_VALUE)-heritage);
    const used = ATTRIBUTE_KEYS.reduce((sum,k)=>{
      const playerValue = Math.max(BASE_ATTRIBUTE_VALUE,numberValue(values[`${k}_base`],BASE_ATTRIBUTE_VALUE)-numberValue(values[`${k}_heritage_applied`],0));
      return sum + Math.max(0,playerValue-BASE_ATTRIBUTE_VALUE);
    },0);
    if(delta>0 && (used>=ATTRIBUTE_POINT_BUDGET || currentPlayer>=MAX_PLAYER_ATTRIBUTE)) return;
    if(delta<0 && currentPlayer<=BASE_ATTRIBUTE_VALUE) return;
    const nextPlayer = Math.max(BASE_ATTRIBUTE_VALUE,Math.min(MAX_PLAYER_ATTRIBUTE,currentPlayer+delta));
    setAttrs({[`${key}_base`]:nextPlayer+heritage},{silent:true},()=>{
      recalculateCharacter();
      refreshCreationBudgets();
      refreshBuilderDisplay();
    });
...
const adjustBuilderSkill = (skill,delta) => {
  const [baseField] = SKILL_ATTRIBUTE_MAP[skill];
  const skillFields = Object.values(SKILL_ATTRIBUTE_MAP).map(v=>v[0]);
  getAttrs(skillFields, values => {
    const current = Math.max(BASE_SKILL_VALUE,numberValue(values[baseField],BASE_SKILL_VALUE));
    const used = skillFields.reduce((sum,field)=>sum+Math.max(0,numberValue(values[field],BASE_SKILL_VALUE)-BASE_SKILL_VALUE),0);
    if(delta>0 && used>=SKILL_POINT_BUDGET) return;
    if(delta<0 && current<=BASE_SKILL_VALUE) return;
    const next = Math.max(BASE_SKILL_VALUE,Math.min(100,current+delta));
    setAttrs({[baseField]:next},{silent:true},()=>{
      recalculateCharacter();
      refreshCreationBudgets();
      refreshBuilderDisplay();
    });
...
const validateCharacterBuilder = (callback=null) => {
  ...
  getAttrs(fields, values => {
    ...
    ATTRIBUTE_KEYS.forEach(key => {
      ...
      const label = key === "wisdom" ? "Agility" : key.charAt(0).toUpperCase()+key.slice(1);
    ...
    Object.entries(SKILL_ATTRIBUTE_MAP).forEach(([skill,[field]]) => {
      if (numberValue(values[field],BASE_SKILL_VALUE) < BASE_SKILL_VALUE) {
        issues.push(`${skill.replace(/_/g," ")} is below ${BASE_SKILL_VALUE}%.`);
      }
    });
...
const migrateCharacterCreationState = () => {
  getAttrs(["character_creation_complete","voe_sheet_version"],values=>{
...
const recalculateCharacter = () => {
  ...
  getAttrs([...new Set(fields)], values => {
    ...
    setAttrs(updates, {silent:true}, refreshBuilderDisplay);
  });
};
...
on("sheet:opened",migrateCharacterCreationState);
...
ATTRIBUTE_KEYS.forEach(k=>bindAttributeTest(k,k==="wisdom"?"Agility":k.charAt(0).toUpperCase()+k.slice(1)));
