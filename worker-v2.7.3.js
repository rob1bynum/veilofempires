...
const refreshAgentProtocolLoadoutSummary = () => {
  const primaryFields = [
    "protocol_slot_1_display","protocol_slot_2_display","protocol_slot_3_display"
  ];
  getSectionIDs("repeating_protocols", rowIds => {
    const fields = [...primaryFields];
    rowIds.forEach(rowId => fields.push(`repeating_protocols_${rowId}_protocol_display`));
    getAttrs(fields, values => {
      const names = [];
      primaryFields.forEach(field => {
        const value = (values[field] || "").trim();
        if (value && value !== "No Protocol") names.push(value);
      });
      rowIds.forEach(rowId => {
        const field = `repeating_protocols_${rowId}_protocol_display`;
        const value = (values[field] || "").trim();
        if (value && value !== "No Protocol") names.push(value);
      });
      setAttrs({
        agent_protocol_slot_1_text:names[0] || "No Protocol",
        agent_protocol_slot_2_text:names[1] || "No Protocol",
        agent_protocol_slot_3_text:names[2] || "No Protocol",
        agent_protocol_extra_text:names.length > 3 ? names.slice(3).join(" • ") : ""
      }, {silent:true});
    });
  });
};
...
const recalculateCharacter = () => {
  const fields = ["heritage","heritage_mirror","specialty","secondary_specialty","specialty_rank","rank","rank_health_bonus","experience","experience_next","protocol_rank_1","protocol_rank_2","protocol_rank_3","protocol_slot_1","protocol_slot_2","protocol_slot_3", "armor_type", "carry_capacity_base", "health", "health_max", "temporary_health", "temporary_health_max", "temporary_health_applied", ...ATTRIBUTE_KEYS.flatMap(k=>[`${k}_base`,`${k}_heritage_applied`]), ...Object.values(SKILL_ATTRIBUTE_MAP).map(v=>v[0]), ...CONDITION_KEYS.map(k=>`condition_${k}`)];
...
on("remove:repeating_protocols", () => { refreshProtocolBudgetWithRepeating(); refreshAgentProtocolLoadoutSummary(); });
