...
const rollActiveWeaponDamage = () => {
  getAttrs(["character_name","active_weapon"],values=>{
    ...
    startRoll(
      `&{template:voe} {{name=${escapeTemplate(weapon.name)}}} {{agent=${escapeTemplate(values.character_name||"Unnamed Agent")}}} {{kind=Damage}} {{rolled=[[${expression}]]}} {{result=[[0]]}} {{description=${escapeTemplate(weapon.type)} damage • ${escapeTemplate(weapon.traits)}}}`,
      results=>{
        const rolled = Math.max(0,Math.trunc(results.results.rolled.result));
        finishRoll(results.rollId,{result:rolled});
      }
    );
  });
};
