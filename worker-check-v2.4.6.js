...
const protocolLibrarySummary = protocolId => {
  const protocol = PROTOCOLS[protocolId] || PROTOCOLS[""];
  return {
    display: protocol.name || "No Protocol",
    summary: protocol.summary || "No Protocol prepared."
  };
};
...
on("change:repeating_protocols:protocol_name", eventInfo => {
  ...
  getAttrs([`${prefix}_protocol_name`], values => {
    const selected = protocolLibrarySummary(values[`${prefix}_protocol_name`] || "");
    setAttrs({
