...
function on(events,cb){String(events).split(/\s+/).filter(Boolean).forEach(e=>(handlers[e] ||= []).push(cb));}
function getAttrs(fields,cb){const o={};fields.forEach(f=>o[f]=attrs[f]??"");cb(o);}
function setAttrs(updates,options,cb){if(typeof options==="function"){cb=options;options={};}Object.assign(attrs,updates);if(cb)cb();}
function getSectionIDs(section,cb){cb([]);} function generateRowID(){return "row1";}
function startRoll(expr,cb){cb({rollId:"1",results:{result:42}});} function finishRoll(){}
function removeRepeatingRow(){} function getTranslationByKey(k){return k;} function setDefaultToken(){}
function trigger(name,info={}){(handlers[name]||[]).forEach(fn=>fn(info));}
...
on("clicked:refresh_builder_review",()=>refreshBuilderFinalReview(()=>postNotice("Review Refreshed","The final character review has been rebuilt from the current character data.","Builder")));
...
attrs.builder_step="7";
attrs.heritage="rajah";attrs.heritage_mirror="rajah";
attrs.affiliation_city="imperium";attrs.affiliation="house_veylan";
attrs.specialty="investigator";
attrs.protocol_slot_1="clockwork_precision";attrs.protocol_rank_1="1";
attrs.protocol_slot_2="ghost_step";attrs.protocol_rank_2="1";
attrs.protocol_slot_3="iron_heart";attrs.protocol_rank_3="1";
attrs.builder_protocol_points_used="3";attrs.builder_protocol_points_available="3";
attrs.attribute_points_used="10";attrs.skill_points_used="30";
attrs.rank="1";attrs.health="10";attrs.health_max="10";
attrs.strength_base="7";attrs.dexterity_base="7";attrs.endurance_base="7";
attrs.intelligence_base="7";attrs.wisdom_base="6";attrs.charisma_base="6";
attrs.athletics="10";attrs.acrobatics="10";attrs.stealth="10";
attrs.investigation="10";attrs.arcana="10";attrs.engineering="10";
attrs.medicine="5";attrs.survival="5";attrs.skill_perception="5";
attrs.insight="5";attrs.persuasion="5";attrs.deception="5";attrs.intimidation="5";
attrs.sleight_of_hand="5";attrs.technology="5";attrs.animal_handling="5";
attrs.close_combat="5";attrs.ranged_combat="5";
trigger("clicked:refresh_builder_review",{});
if(attrs.builder_review_heritage_value!=="Rajah") throw new Error("Heritage review failed");
if(attrs.builder_review_affiliation_value!=="House Veylan") throw new Error("Affiliation review failed");
if(attrs.builder_review_specialty_value!=="Investigator") throw new Error("Specialty review failed");
if(attrs.builder_review_signature_value!=="Clockwork Precision") throw new Error("Signature review failed");
if(attrs.builder_review_protocol_1_name!=="Clockwork Precision") throw new Error("Protocol 1 failed");
if(attrs.builder_review_protocol_2_name!=="Ghost Step") throw new Error("Protocol 2 failed");
if(attrs.builder_review_protocol_3_name!=="Iron Heart") throw new Error("Protocol 3 failed");
if(Number(attrs.builder_review_skill_athletics)<=0) throw new Error("Athletics review failed");
console.log(JSON.stringify({
 heritage:attrs.builder_review_heritage_value,
 affiliation:attrs.builder_review_affiliation_value,
 specialty:attrs.builder_review_specialty_value,
 signature:attrs.builder_review_signature_value,
 protocols:[attrs.builder_review_protocol_1_name,attrs.builder_review_protocol_2_name,attrs.builder_review_protocol_3_name],
 athletics:attrs.builder_review_skill_athletics
}));
