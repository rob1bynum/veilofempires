...
const refreshReactionState = () => {
  getSectionIDs("repeating_protocols",rowIds=>{
    ...
    getAttrs(fields,values=>{
      ...
      const glimpseRank = loadedProtocolRank("glimpse_future",values,rowIds);
      const selected = values.selected_reaction || "none";
      const valid = selected === "glimpse_future" && glimpseRank > 0;
      setAttrs({
        reaction_status_text:available ? "Available" : "Used",
        selected_reaction:valid ? selected : "none",
        selected_reaction_modifier:valid ? Math.min(15,glimpseRank*5) : 0,
        reaction_glimpse_rank:glimpseRank
      },{silent:true});
    });
  });
};
...
const useSelectedReaction = () => {
  getAttrs([
    ...
    if ((values.selected_reaction || "none") !== "glimpse_future") {
      return postNotice("Reaction","No available Reaction ability is selected.","Reaction");
    }
    const modifier = Math.max(0,Math.trunc(numberValue(values.selected_reaction_modifier,0)));
    if (modifier <= 0) {
      return postNotice("Reaction","Glimpse the Future is not currently loaded.","Reaction");
    }

    const mode = values.selected_reaction_mode || "forewarned";
    const modeLabel = REACTION_LIBRARY.glimpse_future.modes[mode];
    const effect = mode === "exploit"
      ? `Your next attack gains +${modifier}%.`
      : `One attack against you suffers -${modifier}%.`;

    setAttrs({
      reaction_available:0,
      reaction_status_text:"Used",
      reaction_pending_attack_bonus:mode==="exploit" ? modifier : 0,
      reaction_pending_defense_penalty:mode==="forewarned" ? -modifier : 0
    });

    startRoll(
      `&{template:voe} {{name=Reaction}} {{agent=${escapeTemplate(values.character_name||"Unnamed Agent")}}} {{kind=Glimpse the Future}} {{description=${escapeTemplate(modeLabel)} • ${escapeTemplate(effect)} • Reaction Spent}}`,
...
on("change:reaction_available change:selected_reaction change:selected_reaction_mode change:protocol_slot_1 change:protocol_rank_1 change:protocol_slot_2 change:protocol_rank_2 change:protocol_slot_3 change:protocol_rank_3",refreshReactionState);
