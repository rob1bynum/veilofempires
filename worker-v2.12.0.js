...
const runInitiativeSkill = (skillKey, label) => {
  ...
  getAttrs([
    ...
    const target = clampPercent(numberValue(values[totalField],0));
    const active = getActiveConditions(values);
    const conditionModifier =
      conditionPenaltyForSkill(skillKey,SKILL_ATTRIBUTE_MAP[skillKey][1],active);
    const finalTarget = clampPercent(target + conditionModifier);

    setAttrs({initiative_popup_open:0},{silent:true});
    runPercentileTest({
      name:`${label} Initiative`,
      target:finalTarget,
      kind:"Initiative",
      description:conditionModifier
        ? `${label} ${target}% | Conditions ${conditionModifier>=0?"+":""}${conditionModifier}% | Final ${finalTarget}%`
        : `${label} Initiative • Target ${finalTarget}%`
    });
  });
};
